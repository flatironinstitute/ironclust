function S_rhd = import_rhd(vcFile_rhs)

% based on read_Intan_RHD2000_file.m
% Modified by James Jun, 2019 Mar 27
...
if (data_present)
    ...
    aux_input_data = zeros(num_aux_input_channels, num_aux_input_samples, 'uint16');
    supply_voltage_data = zeros(num_supply_voltage_channels, num_supply_voltage_samples, 'uint16');
    temp_sensor_data = zeros(num_temp_sensor_channels, num_supply_voltage_samples, 'uint16');
...
if (num_amplifier_channels > 0)
    ...
    if (data_present)
        ...
        struct_add_(t_amplifier);
    end
    struct_add_(spike_triggers);
end
if (num_aux_input_channels > 0)
    struct_add_(aux_input_channels);
    if (data_present)
        struct_add_(aux_input_data);
        struct_add_(t_aux_input);
    end
end
if (num_supply_voltage_channels > 0)
    struct_add_(supply_voltage_channels);
    if (data_present)
        struct_add_(supply_voltage_data);
        struct_add_(t_supply_voltage);
    end
end
if (num_board_adc_channels > 0)
...
if (num_temp_sensor_channels > 0)
    if (data_present)
        struct_add_(temp_sensor_data);
        struct_add_(t_temp_sensor);
    end
end
