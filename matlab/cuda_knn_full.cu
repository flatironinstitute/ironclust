 ...
 * K: 1..K nearest neighbor (knn x nA)
 * I: Index (knn x nA)
 * F: Feature matrix (nC x *)
 * viB: Index B (nB x 1)
 * viA: Index A (nA x 1)
 */
__global__ void cuda_knn_full(float *K, int *I, float const *B, float const *A, const int *vnConst){
    ...
    // shared_mem_byte = 4*(NC*CHUNK + NTHRADS*CHUNK*2)
    ...
    if (tx < CHUNK){           
        if (iA < nA){
            ...
            for (int iK=0; iK<knn; ++iK){
                ...
                K[iK + knn*iA] = sqrt(ABS(sD[iK][iA_]));
            }
        }        
    } // if
} // func
