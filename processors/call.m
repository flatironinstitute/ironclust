%--------------------------------------------------------------------------
% ironclust call library
 
function varargout = call(vcCmd, varargin)
% Memory-efficient version 
% P is static and loaded from file
% Dynamic variables are set in S0=get(0,'UserData')

try
    f_ = str2func(sprintf('%s_', vcCmd));
    switch vcCmd
        % one input, one output
        case {'file2struct', 'prb2geom'}            
            varargout{1} = f_(varargin{1});
            
        % two inputs, one output
        case {'get_set'}
            varargout{1} = f_(varargin{1}, varargin{2}, varargin{3});
            
        otherwise
            error(['unknown command: ', vcCmd]);
    end %switch
catch
    error(['error: ', lasterr()]);
end
...
if isfield(S, vcName), val = S.(vcName); end
if isempty(val), val = def_val; end
end %func


%--------------------------------------------------------------------------
function vcFile_geom = prb2geom_(vcFile_prb)

S_prb = call('file2struct', vcFile_prb);
mrXY = S_prb.geometry;

vcFile_geom = strrep(vcFile_prb, '.prb', '.csv');
csvwrite(vcFile_geom, mrXY);
